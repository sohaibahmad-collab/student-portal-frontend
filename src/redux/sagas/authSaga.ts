...
function* handleLogin(action: PayloadAction<{ email: string; password: string }>) {
  try {
    const user: IUser = yield call([AuthApiClient, AuthApiClient.login], action.payload);
...
function* handleRegister(action: PayloadAction<{ name: string; email: string; password: string }>) {
  try {
    const user: IUser = yield call([AuthApiClient, AuthApiClient.register], action.payload);
